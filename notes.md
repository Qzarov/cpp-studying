...
**Мультиплексирование** (англ. *multiplexing*, *muxing*) - уплотнение канала связи, то есть передача нескольких потоков данных с меньшей скоростью по одному каналу связи. Или иначе: создание в исходном канале связи нескольких подканалов связи с меньшей пропускной способностью.
...
Один слушающий сокет (master) и много одновременно подключенных клиентов (slave-сокеты). Мастер: 
- слушает все сокеты и передает получаемые от них сообщения всем остальным;
...
Для решения задачи существуют следующие способы:

* __select()__
  
   (или pselect()) используется для эффективного слежения за несколькими файловыми дескрипторами — для ожидания, когда какой-то из них не станет «готов», то есть появится возможность чтения-записи данных, или с файловым дескриптором не возникнет «исключительная ситуация». [Подробнее](https://ru.manpages.org/select/2).

   Возможно одновременное выделение не более 1024 дескрипторов (проблема С10К и С100К).

   ```C++
	fd_set set;
	FD_ZERO(&set); // обнуление set
	FD_SET(MasterSocket, &set); // добавление мастер-сокета для того, чтобы иметь возможность принимать соединения
	for (...) {
		FD_SET(*iter, &set); // добавляем слейвов
		int max(); // находим максимальный (последний) дескриптор
		select(max+1, 
				&set, // дескриптор, который слушаем на чтение
				null, // дескриптор, который слушаем на запись
				null, // дескриптор на ошибку
				null // выдерживаемый таймаут
				);
	} 
	for (/* пробегаем по всем слейвам*/) {
		if (FD_ISSET(*iter, &set)) {
			// отправляем сообщение всем остальным
		}
	}
	if (FD_ISSET(MasterSocket, &set)) { accept(); }
   ```
